...
def randomly_spaced_indices(max_len, n_indices, border_size=1, padding=1):
    ...
    if border_size > 0:
        return (
            randomly_spaced_indices(
                max_len - border_size - 2, n_indices, border_size=0, padding=padding
            )
            + border_size
        )
    ...
    while sum(indices) < n_indices:
        ...
        try:
            possible_indices = [
                ...
                if sum(indices[max(0, i - padding) : min(i + 1 + padding, max_len)])
                == 0
            ]
        except:
...
def check_between_objects(obj1, obj2, x, y, padding=0, background=Color.BLACK):
    ...
    if (
        x1 + w1 <= x
        and x < x2
        and y - padding >= max(y1, y2)
        and y + padding < min(y1 + h1, y2 + h2)
    ):
        return True
    ...
    if (
        y1 + h1 <= y
        and y < y2
        and x - padding >= max(x1, x2)
        and x + padding < min(x1 + w1, x2 + w2)
    ):
        return True
...
def orbit(grid, x, y, symmetries):
    ...
    possible_iterations = itertools.product(
        *[list(range(*s._iter_range(grid.shape))) for s in symmetries]
    )
...
class TranslationalSymmetry(Symmetry):
    ...
    def _iter_range(self, grid_shape):
        ...
        if self.translate_y != 0:
            top_of_range = max(
                top_of_range, math.ceil(grid_shape[1] / abs(self.translate_y))
            )
...
def detect_translational_symmetry(grid, ignore_colors=[Color.BLACK], background=None):
    ...
    def score(sym):
        perfectly_preserved, outside_canvas, conflict = _score_symmetry(
            grid, sym, ignore_colors, background=background
        )
...
def _score_symmetry(grid, symmetry, ignore_colors, background=None):
    ...
    if background is None:
        ...
        occupied_locations = np.argwhere(
            (~np.isin(grid, ignore_colors)) & (grid != background)
        )
