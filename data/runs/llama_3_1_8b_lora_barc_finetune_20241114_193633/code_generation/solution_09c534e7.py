...
def transform(input_grid: np.ndarray) -> np.ndarray:
    ...
    objects = find_connected_components(
        input_grid, background=Color.BLACK, monochromatic=False, connectivity=4
    )

    # Identify the main object (the largest connected component)
    object_components = sorted(
        objects, key=lambda obj: np.sum(obj != Color.BLACK), reverse=True
    )
    ...
    boundary_mask[x : x + width, y : y + height] = main_object[
